...
class BjggzyjySpider(scrapy.Spider):
    name = 'bjggzyjy_spider'  # 需要修改成全局唯一的名字
    allowed_domains = ['ggzyfw.beijing.gov.cn']  # 需要修改成爬取网站的域名

    def __init__(self, *args, **kwargs):
        super(BjggzyjySpider, self).__init__(*args, **kwargs)
    ...
    def get_normal_next_page_url(page_index, base_url, ext_param):
        ...
        if page_index == 0:
            return '{}{}/index.html'.format(base_url, ext_param['bid_sort'])
        else:
            return '{}{}/index_{}.html'.format(base_url, ext_param['bid_sort'])
...
class BaseItemCommonParser:
    ...
    def __get_notice_time__(self):
        ...
        try:
            ...
            _notice_time = self.selector.xpath('./div[@class="list-times"]/text()').extract_first()
