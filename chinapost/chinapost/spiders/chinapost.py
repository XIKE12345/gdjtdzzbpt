...
class ChinapostSpider(scrapy.Spider):
    name = 'chinapost_spider'  # 需要修改成全局唯一的名字
    allowed_domains = ['www.cgdcbidding.com']  # 需要修改成爬取网站的域名

    def __init__(self, *args, **kwargs):
        super(ChinapostSpider, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        ...
        # python3  -m scrapy crawl chinapost_spider -a start_time="now" -a end_time="now"
    ...
    def get_normal_next_page_url(page_index, base_url, ext_param):
        ...
        # if page_index == 0:
        #     return '{}{}.htm'.format(base_url, ext_param['bid_sort'])
        # else:
        #     # 'base_url': 'http://www.chinapost.com.cn/html1/category/',
        #     #  'bid_sort': '181392/1929',
            return '{}{}-{}.htm'.format(base_url, ext_param['bid_sort'], page_index + 1)
...
class BaseItemCommonParser:
    ...
    def __get_notice_time__(self):
        ...
        try:
            ...
            _notice_time = self.selector.xpath('./span[@id="ReportIDIssueTime"]/text()').extract_first()
    ...
    def __get_title__(self):
        ...
        :return:
        """
        _ret = self.selector.xpath('./span[@id="ReportIDname"]/a/@title').extract_first().replace('\\n', '').rstrip().lstrip()
