...
class ShanghaiSpider(scrapy.Spider):
    name = 'ccgp_chongqing_spider'  # 需要修改成全局唯一的名字
    allowed_domains = ['www.ccgp-chongqing.gov.cn']  # 需要修改成爬取网站的域名

    def __init__(self, *args, **kwargs):
        super(ShanghaiSpider, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        ...
        # command example:
        #       python3 -m scrapy crawl ccgp_chongqing_spider -a start_time="2019:01:01" -a end_time="2020:02:26"
        ...
        _source_info = {
            ...
            'page_1': {
                ...
                # list页面的base地址
                'base_url': 'https://www.ccgp-chongqing.gov.cn/gwebsite/api/v1/notices/stable/new?',
            ...
            'page_2': {
                ...
                'site_name': '重庆政府采购网',
                'industry': '政府采购',
                'type': '300,302,304,3041,305,306,307,308',
                'time_type': 6 if self.crawl_mode == CrawlMode.HISTORY else 0,
            },
        }
    ...
    def get_normal_next_page_url(page_index, base_url, ext_param):
        _start_time = ext_param['start_time'].replace(':', '-')
        _end_time = ext_param['end_time'].replace(':', '-')
        _param = {
            'endDate': _end_time,
            'pi': page_index + 1,
            'ps': '20',
    ...
    def parse_list_page_common(self, response):
        ...
        if response.meta['param']['requests_type'] == "dict":
            ...
            for selector in _response_data:
                ...
                try:
                    _item_idx += 1
                    _detail_url = 'https://www.ccgp-chongqing.gov.cn/notices/detail/' + str(
                        selector['id']) + '?title=' + selector['title']
...
class BaseItemCommonParser:
    ...
    def __get_notice_time__(self):
        try:
            ...
            _notice_time = self.selector['issueTime']
