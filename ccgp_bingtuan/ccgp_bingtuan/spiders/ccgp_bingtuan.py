...
class Ccgp_bingtuanSpider(scrapy.Spider):
    name = 'ccgp_bingtuan_spider'  # 需要修改成全局唯一的名字
    allowed_domains = ['cgw.xjbt.gov.cn']  # 需要修改成爬取网站的域名

    def __init__(self, *args, **kwargs):
        super(Ccgp_bingtuanSpider, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        ...
        # python3 -m scrapy crawl ccgp_bingtuan_spider -a start_time="2019:01:01" -a end_time="2020:02:25"
        ...
        _source_info = {
            ...
            'page_1': {
                ...
                # list页面的base地址
                'base_url': 'http://cgw.xjbt.gov.cn/cggg/',
                ...
                'xpath_of_list': '//ul[@class="ewb-info-items"]/li',

                # 获得每一个条目链接地址的xpath
                'xpath_of_detail_url': './div[@class="ewb-info-forus"]/a/@href',
...
class BaseItemCommonParser:
    ...
    def __get_notice_time__(self):
        ...
        try:
            ...
            _notice_time = self.selector.xpath('string(./span[@class="ewb-date"])').extract_first().replace('\\n', '').rstrip().lstrip()
    ...
    def __get_title__(self):
        ...
        :return:
        """
        _ret = self.selector.xpath('./div[@class="ewb-info-forus"]/a/text()').extract_first()
