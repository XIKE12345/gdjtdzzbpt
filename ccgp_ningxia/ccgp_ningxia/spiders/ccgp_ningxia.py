...
class Ccgp_ningxia_Spider(scrapy.Spider):
    name = 'ccgp_ningxia_spider'  # 需要修改成全局唯一的名字
    allowed_domains = ['www.ccgp-ningxia.gov.cn']  # 需要修改成爬取网站的域名

    def __init__(self, *args, **kwargs):
        super(Ccgp_ningxia_Spider, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        ...
        # command example:
        # nohup python3 -m scrapy crawl ccgp_ningxia_spider -a start_time="2019:01:01" -a end_time="2020:02:25" > /dev/null&
        ...
        _source_info = {
            ...
            'page_1': {
                ...
                # list页面的base地址
                'base_url': 'http://www.ccgp-ningxia.gov.cn/public/NXGPPNEW/dynamic/contents/CGGG/index.jsp?cid=312&sid=1',
                ...
                'xpath_of_list': '//table[@class="sub-tab"]//tr',

                # 获得每一个条目链接地址的xpath
                'xpath_of_detail_url': './td/a/@href',
        ...
        if response.meta['param']['requests_type'] == "dict":
            ...
            for selector in response.xpath(response.meta['param']['xpath_of_list']):
                ...
                try:
                    ...
                    _detail_url = "http://www.ccgp-ningxia.gov.cn/public/NXGPPNEW/dynamic/" + selector.xpath(
...
class BaseItemCommonParser:
    ...
    def __get_notice_time__(self):
        try:
            ...
            _notice_time = self.selector.xpath('./th/@title').extract_first()
            _ret = _notice_time
        except:
            _ret = ''
    ...
    def __get_title__(self):
        ...
        :return:
        """
        _ret = self.selector.xpath('./td/a/text()').extract_first().split('(')[0]
