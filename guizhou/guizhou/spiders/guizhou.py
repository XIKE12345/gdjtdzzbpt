...
class GuizhouSpider(scrapy.Spider):
    name = 'guizhou_spider'  # 需要修改成全局唯一的名字
    allowed_domains = ['ggzy.guizhou.gov.cn']  # 需要修改成爬取网站的域名

    def __init__(self, *args, **kwargs):
        super(GuizhouSpider, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        ...
        # command example:
        # python3 -m scrapy crawl guizhou_spider -a start_time="2019:01:01" -a end_time="2019:01:02"
        ...
        _source_info = {
            ...
            'page_1': {
                ...
                'xpath_of_list': ['page', 'content'],

                # 获得每一个条目链接地址的xpath
                'xpath_of_detail_url': 'docpuburl',
            ...
            'page_2': {
                ...
                'site_name': '上海市公共资源交易平台',
                'industry': '政府采购',
                'filter[ChannelId]': '5377338,5377101,5377103',
                'filter[inTypeName]': '',
                'time_type': 6 if self.crawl_mode == CrawlMode.HISTORY else 0,
            },
        }
        ...
        for _k, _v in _source_info.items():
            ...
            for _page_num in range(100000):
                ...
                _ext_param = {
                    'filter[ChannelId]': _v['filter[ChannelId]'],
                    'filter[inTypeName]': _v['filter[inTypeName]'],
    ...
    def get_normal_next_page_url(page_index, base_url, ext_param):
        ...
        _param = {
            'filter[ChannelId]': ext_param['filter[ChannelId]'],
            'filter[inTypeName]': ext_param['filter[inTypeName]'],
            'pageNumber': page_index + 1,
            'filter[DOCTITLE-like]': "",
            'filter[docSourceName-like]': "贵州省公共资源交易中心",
    ...
    def parse_list_page_common(self, response):
        ...
        if response.meta['param']['requests_type'] == "dict":
            ...
            for selector in _response_data:
                ...
                try:
                    _item_idx += 1
                    _detail_url = str(selector[response.meta['param']['xpath_of_detail_url']])
...
class BaseItemCommonParser:
    ...
    def get_common_raw_item(self, _id, detail_url, site, ext_param):
        ...
        self.item['notice_type'] = self.__get_notice_type__()
        self.item['notice_type_code'] = self.__get_notice_type_code__()
        self.item['buyer'] = ''
    ...
    def __get_notice_time__(self):
        try:
            ...
            _notice_time = self.selector['DocRelTime']
            _ret = _notice_time.replace('-', '.')
            _ret = _ret[0:10] + " 00:00:00"
        except:
            _ret = ''
    ...
    def __get_notice_type__(self):
        try:
            # 网页格式: 2020-01-20
            # 必须是以下格式: '2020.01.21 10:00:00'
            _ret = self.selector['inTypeName']
        except:
            _ret = ''
            logging.exception('[{}] get_notice_time failed'.format(self.item['_id']))
        return _ret

    def __get_notice_type_code__(self):
        try:
            # 网页格式: 2020-01-20
            # 必须是以下格式: '2020.01.21 10:00:00'
            _ret = self.selector['inTypeName']
    ...
    def __get_notice_type_code__(self):
        try:
            ...
            _notice_code = self.selector['inTypeName']
            if _notice_code != '':
                _ret = _notice_type_code[_notice_code]
            else:
                _ret = '0101'
        except:
            _ret = ''
