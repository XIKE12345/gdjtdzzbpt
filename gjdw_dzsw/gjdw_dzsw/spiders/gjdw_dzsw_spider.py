...
class GjdwDzswSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        _source_info = {
            ...
            'zbgg_search': {
                ...
                # list页面的base地址
                'base_url': 'http://ecp.sgcc.com.cn/topic_project_list.jsp?columnName=topic10',
                ...
                'xpath_of_list': '//table[@class="font02"]/tr',
                # 获得每一个条目链接地址的xpath
                'xpath_of_detail_url': './td[3]/a/@onclick',
            ...
            'zbhxr_search': {
                ...
                # list页面的base地址
                'base_url': 'http://ecp.sgcc.com.cn/topic_news_list.jsp?columnName=topic22',
                ...
                # list页面中，获得条目列表的xpath
                'xpath_of_list': '//ul[@class="font02"]/li',
                # 获得每一个条目链接地址的xpath
                'xpath_of_detail_url': './div[1]/a/@onclick',
                ...
                'time_type': 6 if self.crawl_mode == CrawlMode.HISTORY else 0,
                'column_code1': '014001003',
                'column_code2': '014002009',
                'connect_type': 'news',
            },
            'zbjg_search': {
                # 通常会被填充在'source'字段里，有时也可以放在'tos'
                ...
                # list页面的base地址
                'base_url': 'http://ecp.sgcc.com.cn/topic_news_list.jsp?columnName=topic23',
                ...
                'time_type': 6 if self.crawl_mode == CrawlMode.HISTORY else 0,
                'column_code1': '014001007',
                'column_code2': '014002003',
                'connect_type': 'news',
            },
            'fjgg_search': {
                # 通常会被填充在'source'字段里，有时也可以放在'tos'
                ...
                # list页面的base地址
                'base_url': 'http://ecp.sgcc.com.cn/topic_news_list.jsp?columnName=topic24',
                ...
                'time_type': 6 if self.crawl_mode == CrawlMode.HISTORY else 0,
                'column_code1': '014001005',
                'column_code2': '014002006',
                'connect_type': 'news',
            },
        ...
        for _k, _v in _source_info.items():
            ...
            for _page_num in range(100):
                ...
                _request_url = _v['get_next_page_url'](page_index=_page_num, base_url=_v['base_url'],
                                                       connect_type=_v['connect_type'], ext_param=_ext_param)
    ...
    @staticmethod
    def get_normal_next_page_url(page_index, base_url, connect_type, ext_param):
        """
        ...
        if connect_type == 'project':
            ...
            return '{}&{}'.format(base_url, urlencode(_param1, quote_via=quote_plus))
    ...
    def parse_list_page_common(self, response):
        ...
        for selector in response.xpath(response.meta['param']['xpath_of_list']):
            ...
            try:
                # url 标签
                onc = selector.xpath(response.meta['param']['xpath_of_detail_url'])
                ...
                if len(onc) > 0:
                    ...
                    if response.meta['param']['connect_type'] == 'project':
                        ...
                        _detail_url = peoject_default.format(detail_param[0], detail_param[1])
                    else:
                        news_default = 'http://ecp.sgcc.com.cn/html/news/{}/{}.html'
                        _detail_url = news_default.format(detail_param[0], detail_param[1])
                else:
                    continue
                _item_idx += 1
...
class BaseItemCommonParser:
    ...
    def get_common_raw_item(self, _id, detail_url, site, ext_param):
        ...
        self.item['notice_time'] = self.__get_notice_time__(ext_param['connect_type'])
        self.item['buyer'] = self.__get_buyer__()
        self.item['title'] = self.__get_title__(ext_param['connect_type'])
    ...
    def __get_notice_time__(self, connect_type):
        try:
            # 网页格式: 2020-01-20
            # 必须是以下格式: '2020.01.21 10:00:00'
            notice_time = ''
            if connect_type == 'project':
                notice_time = self.selector.xpath('./td[4]/text()').extract_first().replace('-',
                                                                                            '.').rstrip().lstrip()
            else:
                notice_time = self.selector.xpath('./div[2]/text()').extract_first().replace('-',
                                                                                             '.').rstrip().lstrip()
            if notice_time is not None or '' != notice_time:
                _ret = notice_time + ' 00:00:00'
            else:
                _ret = ''
        except:
            _ret = ''
    ...
    def __get_title__(self, connect_type):
        """
        正文标题
        ...
        _ret = ''
        if connect_type == 'project':
            _ret = self.selector.xpath('./td[3]/a/@title').extract_first().replace('[', '').replace(']', '').replace(
                '【', '') \
                .replace('】', '').rstrip().lstrip()
        else:
            _ret = self.selector.xpath('./div[1]/a/@title').extract_first().replace('[', '').replace(']', '').replace(
                '【', '') \
                .replace('】', '').rstrip().lstrip()
        return _ret
