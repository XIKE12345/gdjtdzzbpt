...
class Ccgp_hainanSpider(scrapy.Spider):
    name = 'ccgp_hainan_spider'  # 需要修改成全局唯一的名字
    allowed_domains = ['www.ccgp-hainan.gov.cn']  # 需要修改成爬取网站的域名

    def __init__(self, *args, **kwargs):
        super(Ccgp_hainanSpider, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        ...
        _source_info = {
            ...
            'page_1': {
                ...
                # list页面的base地址
                'base_url': 'http://www.ccgp-hainan.gov.cn/cgw/cgw_list.jsp?',
    ...
    def get_normal_next_page_url(page_index, base_url, ext_param):
        ...
        if page_index == 0:
            _param = {
                'bid_type': ext_param['bid_type'],
                'zone': ''
            }
            # http://www.ccgp-hainan.gov.cn/cgw/cgw_list.jsp?bid_type=100&zone=
            ...
            # http://www.ccgp-hainan.gov.cn/cgw/cgw_list.jsp?currentPage=2&begindate=&enddate=&title=&bid_type=100&proj_number=&zone=&ctype=
            return '{}?{}'.format(base_url, urlencode(_param, quote_via=quote_plus))
...
class BaseItemCommonParser:
    ...
    def __get_notice_time__(self):
        ...
        try:
            ...
            _notice_time = self.selector.xpath('./i/text()').extract_first()
    ...
    def __get_title__(self):
        ...
        :return:
        """
        _ret = self.selector.xpath('./em/a/text()').extract_first().replace('\\n', '').rstrip().lstrip()
