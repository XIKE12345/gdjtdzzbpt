...
class GdsggzyjyptSpider(scrapy.Spider):
    name = 'gdsggzyjypt_spider'  # 需要修改成全局唯一的名字
    allowed_domains = ['bs.gdggzy.org.cn']  # 需要修改成爬取网站的域名

    def __init__(self, *args, **kwargs):
        super(GdsggzyjyptSpider, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        ...
        # command example:
        # python3 -m scrapy crawl gdsggzyjypt_spider -a start_time="2018:01:01" -a end_time="2019:01:02"
        ...
        _source_info = {
            ...
            'page_1': {
                ...
                # list页面的base地址
                'base_url': 'http://bs.gdggzy.org.cn/osh-web/project/projectbulletin/bulletinList',
        ...
        for _k, _v in _source_info.items():
            ...
            for _page_num in range(100000):
                ...
                if _v["method"] == "post":
                    _payload = {
                        'orgCode': "",
                        'tradeTypeId': _v['tradeTypeId'],
                        'queryType': _v['queryType'],
                        'tradeItemId': _v['tradeItemId'],
                        'bulletinName': "",
                        'startTime': str(self.start_time),
                        'endTime': str(self.end_time),
                        'pageNum': str(_page_num + 1)
                    }
...
class BaseItemCommonParser:
    ...
    def __get_notice_time__(self):
        try:
            ...
            _notice_time = self.selector.xpath('string(./td[4])').extract_first().rstrip().lstrip().split('\r\n')[0]
    ...
    def __get_title__(self):
        ...
        :return:
        """
        _ret = self.selector.xpath('./td[2]/@title').extract_first().replace('\\n', '').rstrip().lstrip()
