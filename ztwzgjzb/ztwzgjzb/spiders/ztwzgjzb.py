...
class ZtwzgjzbSpider(scrapy.Spider):
    name = 'ztwzgjzb_spider'  # 需要修改成全局唯一的名字
    allowed_domains = ['bidding.crmsc.com.cn']  # 需要修改成爬取网站的域名

    def __init__(self, *args, **kwargs):
        super(ZtwzgjzbSpider, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        ...
        # command example:
        # python3 -m scrapy crawl ztwzgjzb_spider -a start_time="2019:01:01" -a end_time="2019:01:02"
        ...
        _source_info = {
            ...
            'page_1': {
                ...
                # list页面的base地址
                'base_url': 'https://bidding.crmsc.com.cn/bulletin/list',
            ...
            # 'page_2': {
            #     # 通常会被填充在'source'字段里，有时也可以放在'tos'
            #     'name': '内蒙古能建电子招标投标交易平台',
            #
            #     # list页面的base地址
            #     'base_url': 'http://jc.imeec.cn/webportal/index/bidnotice/getBidNoticeList.do',
            #
            #     # list页面的call_back处理函数
            #     'callback': self.parse_list_page_common,
            #
        ...
        for _k, _v in _source_info.items():
            ...
            for _page_num in range(100000):
                ...
                # 生成request
                if _v["method"] == "post":
                    payload = '{"pageSize":15,"currentPage":"2","bulletinType":"","bidType":"","purchaseMode":"","time":"0","key":""}'
                    # payload = {
                    #     'pageSize': "15",
                    #     "currentPage": str(_page_num + 1),
                    #     'bulletinType': "",
                    #     'bidType': "",
                    #     'purchaseMode': _v['purchaseMode'],
                    #     'time': "0",
                    #     'key': ""
                    # }
    ...
    def parse_list_page_common(self, response):
        ...
        if response.meta['param']['requests_type'] == "dict":
            ...
            for selector in _response_data:
                ...
                try:
                    _item_idx += 1
                    _detail_url = 'https://bidding.crmsc.com.cn/bulletin/look/' + str(
...
class BaseItemCommonParser:
    ...
    def __get_notice_time__(self):
        try:
            ...
            _ret = self.selector['saleBeginTime']
            _ret = _ret.replace('-', '.')
        except:
            _ret = ''
