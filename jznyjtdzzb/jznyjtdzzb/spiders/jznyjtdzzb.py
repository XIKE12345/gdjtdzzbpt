...
class JznyjtdzzbSpider(scrapy.Spider):
    name = 'jznyjtdzzb_spider'  # 需要修改成全局唯一的名字
    allowed_domains = ['www.jzbidding.com']  # 需要修改成爬取网站的域名

    def __init__(self, *args, **kwargs):
        super(JznyjtdzzbSpider, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        ...
        # python3  -m scrapy crawl jznyjtdzzb_spider -a start_time="now" -a end_time="now"
...
class BaseItemCommonParser:
    ...
    def __get_notice_time__(self):
        ...
        try:
            ...
            _notice_time = self.selector.xpath('string(./a/div[@class="newsDate"])').extract_first().strip().replace(
                '\r\n         ', '/')
    ...
    #             'cid=21': '煤炭产能置换指标交易'
    #         }
    #         # http://jznyjtdzzb.gov.cn/trade/noticeInfo/023061D4-B201-4FDD-A671-4DD22393EF9C?cid=21&type=0
    ...
    def __get_title__(self):
        ...
        :return:
        """
        _ret = self.selector.xpath('string(./a/h1[1])').extract_first().replace('\\n', '').rstrip().lstrip().split('【')[
            -1].split('】')[0]
        return _ret
