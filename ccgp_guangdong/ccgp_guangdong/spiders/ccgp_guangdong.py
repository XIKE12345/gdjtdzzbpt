...
class Ccgp_guangdongSpider(scrapy.Spider):
    name = 'ccgp_guangdong_spider'  # 需要修改成全局唯一的名字
    allowed_domains = ['gpcgd.gd.gov.cn']  # 需要修改成爬取网站的域名

    def __init__(self, *args, **kwargs):
        super(Ccgp_guangdongSpider, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        ...
        # command example:
        # python3  -m scrapy crawl ccgp_guangdong_spider -a start_time="2019:01:01" -a end_time="2020:02:26"
        # python3  -m scrapy crawl ccgp_guangdong_spider -a start_time="now" -a end_time="now"
        ...
        _source_info = {
            ...
            'page_1': {
                ...
                'xpath_of_list': '//div[@class="pub_cont06"]/div/li',

                # 获得每一个条目链接地址的xpath
                'xpath_of_detail_url': './span[@class="span_tit"]/a/@href',
            ...
            'page_2': {
                ...
                'content_code': '1',
                'industryName': '',
                'bid_type': 'gpcgd_buy_bidding_res',
                'time_type': 6 if self.crawl_mode == CrawlMode.HISTORY else 0,
            },
            'page_3': {
                # 通常会被填充在'source'字段里，有时也可以放在'tos'
            ...
            'page_6': {
                ...
                'content_code': '1',
                'industryName': '',
                'bid_type': 'gpcgd_buy_bidding_candidate',
                'time_type': 6 if self.crawl_mode == CrawlMode.HISTORY else 0,
            },
    ...
    def get_normal_next_page_url(page_index, base_url, ext_param):
        ...
        if page_index == 0:
            ...
            return '{}{}/index.html'.format(base_url, ext_param['bid_type'])
        else:
            # http://gpcgd.gd.gov.cn/gpcgd_buy_info/index_2.html
            return '{}{}/index_{}'.format(base_url, ext_param["bid_type"], page_index + 1)
    ...
    def parse_list_page_common(self, response):
        ...
        for selector in response.xpath(response.meta['param']['xpath_of_list']):
            ...
            try:
                _item_idx += 1
                #  http://gpcgd.gd.gov.cn/gpcgd_buy_info/content/post_2872738.html
...
class BaseItemCommonParser:
    ...
    def __get_notice_time__(self):
        ...
        try:
            ...
            _notice_time = self.selector.xpath('string(./span[@class="span_time"])').extract_first().replace('\n\t\t\t\t\t\t\t','').replace('\n\t\t\t\t\t\t','')
    ...
    def __get_title__(self):
        ...
        :return:
        """
        _ret = self.selector.xpath('string(./span[@class="span_tit"]/a)').extract_first()
