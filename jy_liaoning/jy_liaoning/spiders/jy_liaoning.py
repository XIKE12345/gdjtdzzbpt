...
class Jy_liaoningSpider(scrapy.Spider):
    name = 'jy_liaoning_spider'  # 需要修改成全局唯一的名字
    allowed_domains = ['www.lntb.gov.cn']  # 需要修改成爬取网站的域名

    def __init__(self, *args, **kwargs):
        super(Jy_liaoningSpider, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        ...
        # command example:
        # python3  -m scrapy crawl jy_liaoning_spider -a start_time="2019:01:01" -a end_time="2020:02:25"
        # python3  -m scrapy crawl jy_liaoning_spider -a start_time="now" -a end_time="now"
        ...
        _source_info = {
            ...
            'page_1': {
                ...
                # list页面的base地址
                'base_url': 'http://www.lntb.gov.cn/Article_Class2.asp?',
                ...
                'xpath_of_list': '//td[@height="200"]//a',

                # 获得每一个条目链接地址的xpath
                'xpath_of_detail_url': './@href',
    ...
    def get_normal_next_page_url(page_index, base_url, ext_param):
        if page_index == 0:
            return '{}ClassID={}'.format(base_url, ext_param['ClassID'])
        else:
            _param = {
                'ClassID': ext_param['ClassID'],
                'SpecialID': "0",
                'page': page_index + 1,
            }
    ...
    def parse_list_page_common(self, response):
        ...
        for selector in response.xpath(response.meta['param']['xpath_of_list']):
            ...
            try:
                _item_idx += 1
                _detail_url = 'http://www.lntb.gov.cn/' + str(selector.xpath(response.meta['param']['xpath_of_detail_url']).extract_first())
...
class BaseItemCommonParser:
    ...
    def __get_notice_time__(self):
        ...
        try:
            ...
            _notice_time = self.selector.xpath('//td[@height="200"]/font/text()').extract_first().replace('年', '.').replace('月', '.').replace(
                '日', '.')
            _ret = _notice_time + ' 00:00:00'
    ...
    def __get_title__(self):
        ...
        :return:
        """
        _ret = self.selector.xpath('./text()').extract_first().rstrip().lstrip()
