...
# 以下是杰云相关的包
sys.path.append('..')
from sourthPower.sourthPower.common import JyScrapyUtil
from sourthPower.sourthPower.common import JyCrawlHelper
from sourthPower.sourthPower.common import CommonRawItem
...
class SouthernPowerSpider(scrapy.Spider):
    name = 'southern_power_spider'                    # 需要修改成全局唯一的名字
    allowed_domains = ['bidding.csg.cn']    # 需要修改成爬取网站的域名

    def __init__(self, *args, **kwargs):
        super(SouthernPowerSpider, self).__init__(*args, **kwargs)
    ...
    def get_normal_next_page_url(page_index, base_url):
        ...
        _param = {
            ...
            # 'dbselect': 'bidx',
            # 'kw': '',
            # 'start_time': ext_param['start_time'],
            # 'end_time': ext_param['end_time'],
            # 'timeType': ext_param['time_type'],
            # 'displayZone': '',
            # 'zoneId': '',
            # 'pppStatus': 0,
            # 'agentName': '',
        ...
        return '{}index_{}.jhtml'.format(base_url, urlencode(_param, quote_via=quote_plus))
...
class BaseItemCommonParser:
    ...
    def __get_area__(self):
        try:
            _ret = self.selector.xpath('./span/a[1]/text()').extract_first().replace('\\n', '').rstrip().lstrip()
        except:
            _ret = ''
    ...
    def __get_notice_time__(self):
        ...
        try:
            ...
            _bid_info = self.selector.xpath('./span[@class="Right Gray"]/text()')
            # if _bid_info:
            #     _a = _bid_info.split('|')
            #     if len(_a) > 0:
            #         _ret = _a[0].replace('\\n', '').rstrip().lstrip()
    ...
    def __get_buyer__(self):
        try:
            _ret = ''
            _bid_info = self.selector.xpath('./span/text()').extract_first()
            if _bid_info:
                _a = _bid_info.split('|')
                if len(_a) > 1:
                    _ret = _a[1].replace('\\n', '').rstrip().lstrip()
                    if '采购人：' in _ret:
                        _ret = _ret.replace('采购人：', '')
        except:
            _ret = ''
            logging.exception('[{}] get_area_detail failed'.format(self.item['_id']))
    ...
    def __get_notice_type__(self):
        try:
            _ret = self.selector.xpath('.//span/strong/text()').extract_first().replace('\\n', '').strip()
        except:
            _ret = ''
    ...
    def __get_title__(self):
        ...
        :return:
        """
        _ret = self.selector.xpath('./a/@href/text()').rstrip().lstrip()
