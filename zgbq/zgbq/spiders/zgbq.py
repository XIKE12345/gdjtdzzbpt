...
class ZgbqSpider(scrapy.Spider):
    name = 'zgbq_spider'  # 需要修改成全局唯一的名字
    allowed_domains = ['www.cse-bidding.com']  # 需要修改成爬取网站的域名

    def __init__(self, *args, **kwargs):
        super(ZgbqSpider, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        ...
        # python3  -m scrapy crawl zgbq_spider -a start_time="now" -a end_time="now"
    ...
    def get_normal_next_page_url(page_index, base_url, ext_param):
        if ext_param['bid_sort'] == 'belletinList':
            if page_index == 0:
                _param = {
                    'searchDate': '1995-02-06',
                    'dates': 300,
                    'word': '',
