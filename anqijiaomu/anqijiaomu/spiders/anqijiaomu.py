...
class AnqijiaomuSpider(scrapy.Spider):
    name = 'anqijiaomu_spider'  # 需要修改成全局唯一的名字
    allowed_domains = ['anqijiaomu.sinopec.com']  # 需要修改成爬取网站的域名

    def __init__(self, *args, **kwargs):
        super(AnqijiaomuSpider, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        ...
        # command example:
        # python3 -m scrapy crawl anqijiaomu_spider -a start_time="2019:01:01" -a end_time="2019:01:02"
        ...
        for _k, _v in _source_info.items():
            ...
            for _page_num in range(100000):
                ...
                if _v["method"] == "post":
                    payload = {
                        '_request_data': _v['_request_data']
                    }
    ...
    def parse_list_page_common(self, response):
        ...
        if response.meta['param']['requests_type'] == "dict":
            ...
            for selector in _response_data:
                ...
                try:
                    _item_idx += 1
                    _detail_url = 'https://angel.going-link.com/' + str(
...
class BaseItemCommonParser:
    ...
    def __get_notice_time__(self):
        try:
            ...
            _ret = self.selector['release_date']
            _ret = _ret.replace('-', '.')
        except:
            _ret = ''
