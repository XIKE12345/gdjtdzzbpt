...
class JznyjtSpider(scrapy.Spider):
    name = 'jznyjt_spider'  # 需要修改成全局唯一的名字
    allowed_domains = ['jznyjt.gov.cn']  # 需要修改成爬取网站的域名

    def __init__(self, *args, **kwargs):
        super(JznyjtSpider, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        ...
        # python3  -m scrapy crawl jznyjt_spider -a start_time="now" -a end_time="now"
    ...
    def get_normal_next_page_url(page_index, base_url, ext_param):
        # http://www.jzbidding.com/jznycms/category/belletinList.html?dates=300&categoryId=88&page=1&tabName=招标公告
        if page_index == 0:
            _param = {
                'dates': "300",
            ...
            return '{}{}.html?{}'.format(base_url, ext_param['bid_type'], urlencode(_param, quote_via=quote_plus))
        else:
            # http://www.jzbidding.com/jznycms/category/belletinList.html?searchDate=1995-02-14&dates=300&word=&categoryId=88&industryName=&status=&page=2
            _param = {
                'searchDate': "1995-02-14",
                'dates': "300",
                'word': "",
                'categoryId': ext_param['categoryId'],
                'industryName': "",
...
class BaseItemCommonParser:
    ...
    def __get_notice_time__(self):
        ...
        try:
            ...
            _notice_time = self.selector.xpath('string(./a/div[@class="newsDate"]/text())').extract_first().replace('\r\n         ','')
    ...
    #             'cid=21': '煤炭产能置换指标交易'
    #         }
    #         # http://jznyjt.gov.cn/trade/noticeInfo/023061D4-B201-4FDD-A671-4DD22393EF9C?cid=21&type=0
    #         _url_list = detail_url.lstrip('http://').split('/')
    #         _notice_code = _url_list[3].split('?')[1].split('&')[0]
    ...
    def __get_title__(self):
        ...
        :return:
        """
        _ret = self.selector.xpath('string(./a/h1)').extract_first().split("【")[1].split("】")[0]
